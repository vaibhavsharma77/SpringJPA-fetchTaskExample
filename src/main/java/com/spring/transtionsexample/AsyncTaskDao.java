...
public class AsyncTaskDao {
    ...
    public AsyncEntity fetchTask(){
        ...
        Optional<AsyncEntity> open = taskRepository.findByStatusAndAttemptLessThanAndModificationDateLessThan("OPEN",3, LocalDateTime.now().minusSeconds(10));
        if (open.isPresent()){
    ...
    private void updateTaskAfterFetch(AsyncEntity asyncEntity){
        ...
        asyncEntity.setStatus("FETCH");
        asyncEntity.setAttempt(asyncEntity.getAttempt()+1);
        taskRepository.save(asyncEntity);
    }

    @Transactional
    public void updateStatus(String status,AsyncEntity asyncEntity){
